...
public class machine_gun : MonoBehaviour
{
   ...
   public void Start()
    ...
    void Update()
    {
        ...
        if(button_shoot && magazine_current < 1 && !button_aim)
        {
            ani.SetInteger("machine_gun", 0);
        }
        if (button_shoot && magazine_current < 1 && button_aim)
        {
            ani.SetInteger("machine_gun", 4);
        }
    ...
    public IEnumerator shoot()
    {
        ...
        if(magazine_current < 12)
        {
            bullet_1.SetActive(false);
        }
        if (magazine_current < 11)
        {
            bullet_2.SetActive(false);
        }
        if (magazine_current < 10)
        {
            bullet_3.SetActive(false);
        }
        if (magazine_current < 9)
        {
            bullet_4.SetActive(false);
        }
        if (magazine_current < 8)
        {
            bullet_5.SetActive(false);
        }
        if (magazine_current < 7)
        {
        ...
        {
            bullet_9.SetActive(false);
        }
        if (magazine_current < 3)
        {
            bullet_10.SetActive(false);
        }
        if (magazine_current < 2)
        {
            bullet_11.SetActive(false);
        }
        if (magazine_current < 1)
        {
            bullet_12.SetActive(false);
        }
    ...
    public GameObject bullet_7;
    public GameObject bullet_8;
    public GameObject bullet_9;
    public GameObject bullet_10;
    public GameObject bullet_11;
    public GameObject bullet_12;
    ...
    public IEnumerator reload_start()
    {
        ...
        {
            reload_time = 2.415f;
            ani.SetInteger("machine_gun", 5); // unempty reload    - putting aduioclip on new spawned empty object and transform parent it to the point Shoot_start_point for a static sound

            GameObject g = Instantiate(Clip_on_point, Shoot_start_point.transform.position, transform.rotation);
            g.GetComponent<AudioSource>().clip = reload_audio;
            g.GetComponent<AudioSource>().maxDistance = 15;
            g.GetComponent<AudioSource>().Play();
            g.GetComponent<timer_destroy>().ticks = 600;
            g.transform.parent = Shoot_start_point.transform;
        }
        else
        {
            reload_time = 2.415f;
            ani.SetInteger("machine_gun", 3); // empty reload
        ...
        bullet_9.SetActive(true);
        bullet_10.SetActive(true);
        bullet_11.SetActive(true);
        bullet_12.SetActive(true);
