...
public class rp9 : MonoBehaviour
{
...
;
    ...
    void FixedUpdate()
    {
        ...
        if (button_aim && !button_shoot && !in_reload)
        {
            ...
            ani.SetInteger("rp9", 4);
        }
        if (!button_aim && !button_shoot && !in_reload)
        {
            // idle animation / animator
            ani.SetInteger("rp9", 0);
        }
    ...
    public IEnumerator reload_start()
    {
        ...
        {
            reload_time = 1.62474f;
            ani.SetInteger("rp9", 5); // unempty reload    - putting aduioclip on new spawned empty object and transform parent it to the point Shoot_start_point for a static sound
        ...
        {
            reload_time = 2.2913f;
            ani.SetInteger("rp9", 3); // empty reload
