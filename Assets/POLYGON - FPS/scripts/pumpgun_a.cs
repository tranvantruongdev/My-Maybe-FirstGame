...
public class pumpgun_a : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (button_aim && !button_shoot && !in_reload && !in_shoot)
        {
            ...
            ani.SetInteger("pumpgun_a", 4);
        }
        if (!button_aim && !button_shoot && !in_reload && !in_shoot)
        {
            // idle animation / animator
            ani.SetInteger("pumpgun_a", 0);
        }
    ...
    public IEnumerator shoot()
    {
        ...
        {
            // aim animation / animator
            ani.SetInteger("pumpgun_a", 5);
        }
        if (!button_aim)
        {
            // idle animation / animator
            ani.SetInteger("pumpgun_a", 9);
        }
    ...
    public IEnumerator reload_start()
    {
        ...
        reload_time = 1f;
        ani.SetInteger("pumpgun_a", 3); // unempty reload    - putting aduioclip on new spawned empty object and transform parent it to the point Shoot_start_point for a static sound
        ...
        // reload time over
        yield return new WaitForSeconds(0.391f);
        bullet_mesh.SetActive(false);
