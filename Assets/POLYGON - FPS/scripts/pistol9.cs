...
public class pistol9 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (button_aim && !button_shoot && !in_reload && magazine_current > 0)
        {
            ...
            ani.SetInteger("pistol9", 5);
        }
        // idel
        if (!button_aim && !button_shoot && !in_reload && magazine_current > 0)
        {
            // idle animation / animator
            ani.SetInteger("pistol9", 0);
        }
        ...
        {
            // aim animation / animator
            ani.SetInteger("pistol9", 6);
        }
        // idel EMPTY
        if (!button_aim && !button_shoot && !in_reload && magazine_current < 1)
        {
            // idle animation / animator
            ani.SetInteger("pistol9", 1);
        }
    ...
    public IEnumerator reload_start()
    {
        ...
        {
            reload_time = 1.374f;
            ani.SetInteger("pistol9", 10); // unempty reload    - putting aduioclip on new spawned empty object and transform parent it to the point Shoot_start_point for a static sound
        ...
        {
            reload_time = 1.166f;
            ani.SetInteger("pistol9", 11); // empty reload
