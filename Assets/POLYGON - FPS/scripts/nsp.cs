...
public class nsp : MonoBehaviour
{
   ...
   public void Start()
    ...
    void Update()
    {
        ...
        if (button_aim && !button_shoot && !in_reload && magazine_current > 0)
        {
            ...
            ani.SetInteger("nsp", 5);
        }
        // idel
        if (!button_aim && !button_shoot && !in_reload && magazine_current > 0)
        {
            // idle animation / animator
            ani.SetInteger("nsp", 0);
        }
        ...
        {
            // aim animation / animator
            ani.SetInteger("nsp", 6);
        }
        // idel EMPTY
        if (!button_aim && !button_shoot && !in_reload && magazine_current < 1)
        {
            // idle animation / animator
            ani.SetInteger("nsp", 1);
        }
    ...
    public IEnumerator reload_start()
    {
        ...
        {
            reload_time = 1.374f;
            ani.SetInteger("nsp", 10); // unempty reload    - putting aduioclip on new spawned empty object and transform parent it to the point Shoot_start_point for a static sound
        ...
        {
            reload_time = 0.999f;
            ani.SetInteger("nsp", 11); // empty reload
