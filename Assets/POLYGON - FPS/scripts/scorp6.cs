...
public class scorp6 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (button_aim && !button_shoot && !in_reload && magazine_current > 0)
        {
            ...
            ani.SetInteger("scorp6", 5);
        }
        // idel
        if (!button_aim && !button_shoot && !in_reload && magazine_current > 0)
        {
            // idle animation / animator
            ani.SetInteger("scorp6", 0);
        }
        ...
        {
            // aim animation / animator
            ani.SetInteger("scorp6", 6);
        }
        // idel EMPTY
        if (!button_aim && !button_shoot && !in_reload && magazine_current < 1)
        {
            // idle animation / animator
            ani.SetInteger("scorp6", 1);
        }
    ...
    public IEnumerator reload_start()
    {
        ...
        {
            reload_time = 1.458f;
            ani.SetInteger("scorp6", 10); // unempty reload    - putting aduioclip on new spawned empty object and transform parent it to the point Shoot_start_point for a static sound
        ...
        {
            reload_time = 2.124f;
            ani.SetInteger("scorp6", 11); // empty reload
