   ...
   public void Start()
    ...
    void Update()
    {
        ...
        if (button_aim && !button_shoot && !in_reload && !in_shoot)
        {
            ...
            ani.SetInteger("shotgun3", 4);
        }
        if (!button_aim && !button_shoot && !in_reload && !in_shoot)
        {
            // idle animation / animator
            ani.SetInteger("shotgun3", 0);
        }
    ...
    public IEnumerator reload_start()
    {
        ...
        reload_time = 0.958f;
        ani.SetInteger("shotgun3", 3); //  reload - putting aduioclip on new spawned empty object and transform parent it to the point Shoot_start_point for a static sound
        ...
        if (magazine_current == magazine_max)
        {
            ...
            {
                // aim animation / animator
                ani.SetInteger("shotgun3", 5);
            }
            if (!button_aim)
            {
                // idle animation / animator
                ani.SetInteger("shotgun3", 9);
            }
