...
public class sce5 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (button_aim && !button_shoot && !in_reload && magazine_current > 0)
        {
            ...
            ani.SetInteger("sce5", 5);
        }
        // idel
        if (!button_aim && !button_shoot && !in_reload && magazine_current > 0)
        {
            // idle animation / animator
            ani.SetInteger("sce5", 0);
        }
        ...
        {
            // aim animation / animator
            ani.SetInteger("sce5", 6);
        }
        // idel EMPTY
        if (!button_aim && !button_shoot && !in_reload && magazine_current < 1)
        {
            // idle animation / animator
            ani.SetInteger("sce5", 1);
        }
        ...
        // walking states not empty magazine /  not accessable while reloading
        if (walking && !in_reload && magazine_current > 0)
        {
            if (!button_aim)
            {
                // idle with walking
                ani.SetInteger("sce5", 3);
        ...
        // walking states empty magazine /  not accessable while reloading
        if (walking && !in_reload && magazine_current < 1)
        {
            if (!button_aim)
            {
                // idle with walking
                ani.SetInteger("sce5", 4);
    ...
    public IEnumerator reload_start()
    {
        ...
        {
            reload_time = 1.249f;
            ani.SetInteger("sce5", 10); // unempty reload    - putting aduioclip on new spawned empty object and transform parent it to the point Shoot_start_point for a static sound
        ...
        {
            reload_time = 1.583f;
            ani.SetInteger("sce5", 11); // empty reload
    ...
    public GameObject sight_a;  // holder for equipments
    ...
    public void cam_equipment()
    {
        ...
        if (button_aim && !running && !in_reload && !red_dot_a_bool && !scope) //aim
        {
    ...
    public void change_equipment()
    {
        ...
        if (red_dot_a_bool || red_dot_b_bool || red_dot_c_bool || red_dot_d_bool || red_dot_e_bool || scope_a_bool || scope_b_bool || scope_c_bool)
        {
            ...
            sight_a.transform.localEulerAngles = new Vector3(-0.028f, 90.00001f, 1);
            sight_b.transform.localEulerAngles = new Vector3(-179.158f, 89.99999f, 1);
            scope = true;
        }
        ...
        if (!red_dot_a_bool && !red_dot_b_bool && !red_dot_c_bool && !red_dot_d_bool && !red_dot_e_bool && !scope_a_bool && !scope_b_bool && !scope_c_bool)
        {
            ...
            sight_a.transform.localEulerAngles = new Vector3(-89.97201f, 0, 91);
            sight_b.transform.localEulerAngles = new Vector3(-89.97201f, 0, 90.00001f);
        }
